				...
				<view v-if="conversation.type !== '@TIM#SYSTEM'" :id="item.ID">
					<view class="t-message-item">
						...
						<view v-if="item.type !== 'TIMGroupTipElem'"
							...
							<image class="t-message-avatar" v-if="item.flow === 'in'" :src="item.avatar || avatarUrls[0].avatarUrl ">
							</image>
							...
							<image class="t-message-avatar" v-if="item.flow === 'out'" :src="avatarUrl || imgSrc  ">
							</image>
						</view>
					</view>
				</view>
	...
	export default {
		data() {
			return {
				...
				triggered: true,
				nextReqMessageID: '',
				avatarUrl: uni.$avatarUrl,
				avatarUrls: uni.$avatarUrls,
				imgSrc:'https://thirdwx.qlogo.cn/mmopen/vi_32/OdkKPeFby5Or5oIlP5EyOup82cDDqBbhCAY3oiaAsOWeJJkibEd3tlWAV6hia0Css2IFVoibUyrFk44OibBlS1tWCFg/132',
				// 下一条消息标志
		...
		onLoad() {
			...
			// console.log(db,'sddfsafdasyfdyafsd')
			// let dataset = db.collection('xiaoli_db');
			// dataset.get().then(res => {
			// 	console.log(res.result.data,'搞撒范德萨分')
			// 	that.avatarUrls = res.result.data;
			// })
		},
		mounted() {
