...
@param uid 用户 ID，建议设置长度1~48，格式同channelId，并保证唯一性。如果不填或设置为nil，SDK 会自动分配一个，并在 joinSuccessBlock 回调方法中返回，App 层必须记住该返回值并维护，SDK 不对该返回值进行维护。
...
/** 指定当前音乐文件的播放音轨。
 ...
 获取音乐文件的音轨索引后，你可以调用该方法指定任一音轨进行播放。例如， 如果一个多音轨文件的不同音轨存放了不同语言的歌曲，则你可以调用该方法设置音乐文件的播放语言。

 **Notes**

 - 你需要在调用 startAudioMixing 并收到 localAudioMixingStateDidChanged 回调后调用该方法。
 - 该方法仅适用于 iOS。
 - 该方法支持的音频文件格式见 RTC SDK 支持播放哪些格式的音频文件。

 @param index 指定的播放音轨。该参数需小于或等于 getAudioTrackCount 的返回值。
 ...
 **Notes**
 ...
 - 你需要在调用 startAudioMixing 并收到 localAudioMixingStateDidChanged(ARAudioMixingStatePlaying) 回调后调用该方法。
 - 该方法仅适用于双声道的音乐文件。

 @param mode 声道模式。详见 ARAudioMixingDualMonoMode 。

  @return - 0方法调用成功，<0方法调用失败
  */
- (int)setAudioMixingDualMonoMode:(ARAudioMixingDualMonoMode)mode;
 ...
 **Note**:
 ...
 @return ≥ 0: 方法调用成功，返回当前音乐文件播放进度（ms）。0 表示当前音乐文件未开始播放。<0方法调用失败
 */
...
/** 获取当前音乐文件时长
 ...
 @return ≥ 0: 方法调用成功，返回当前音乐文件时长 < 0: 方法调用失败
 */
...
/** 创建数据流。
 ...
 @param config 数据流设置：ARDataStreamConfig 。

 @return 创建数据流成功则返回数据流 ID。< 0: 创建数据流失败。
 */
- (int)createDataStream:(NSInteger* _Nonnull)streamId config:(ARDataStreamConfig* _Nonnull)config;
...
/** 获取视频截图
 ...
 @param uid 指定用户的用户 ID
 @param filePath 截图的本地保存路径，需精确到文件名及格式， 例如：iOS: /App Sandbox/Library/Caches/example.jpg 请确保目录存在且可写。
