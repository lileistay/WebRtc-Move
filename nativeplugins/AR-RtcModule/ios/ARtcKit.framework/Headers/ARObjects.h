...
/** 数据流设置。
 ...
 @since 4.3.0.0
 */
__attribute__((visibility("default"))) @interface ARDataStreamConfig : NSObject

/** 是否保证接收到的数据按发送的顺序排列。

 - YES: 保证 SDK 按照发送方发送的顺序输出数据包。
 - NO: 不保证 SDK 按照发送方发送的顺序输出数据包
...
/** 是否与本地发送的音频流同步。
 ...
 设置为与音频同步后，如果数据包的延迟在音频延迟的范围内，SDK 会在播放音频的同时 触发与该音频包同步的 receiveStreamMessageFromUid 回调。 当需要数据包立刻到达接收端时，不能将该参数设置为 YES。建议你仅在需要实现特殊场景，例如歌词同步时，设置为与音频同步。
 */
